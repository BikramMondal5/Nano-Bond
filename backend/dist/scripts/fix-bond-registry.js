...
async function main() {
    console.log("Fixing Bond Registry...");
    ...
    // Find the bond
    const bond = await Bond_1.Bond.findOne({ bondName: 'IIT KGP2' });
    if (!bond) {
        console.error("Bond 'IIT KGP2' NOT FOUND in DB.");
        return;
    }
    const bondAddress = bond.contractAddress;
    console.log(`Bond: ${bond.bondName} (${bondAddress})`);
    const BOND_ABI = [
