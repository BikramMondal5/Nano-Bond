...
async function main() {
    ...
    const treasury = new ethers_1.ethers.Contract(treasuryAddr, TREASURY_ABI, provider);
    try {
        ...
        const bond = new ethers_1.ethers.Contract(bondAddr, BOND_ABI, provider);
        const registryAddr = await bond.registry();
        console.log(`Bond points to Registry: ${registryAddr}`);
        // Check if MATCHES config
        if (registryAddr.toLowerCase() !== config_1.config.contracts.registryAddress.toLowerCase()) {
