...
async function main() {
    ...
    const amount = ethers_1.ethers.parseUnits("500", 6); // 500 USDT
    // Addresses
    ...
    console.log(`Amount: ${ethers_1.ethers.formatUnits(amount, 6)} USDT`);
    // Check Balance
    const usdtAddr = config_1.config.contracts.usdtAddress;
    const usdt = new ethers_1.ethers.Contract(usdtAddr, ["function balanceOf(address) view returns (uint256)"], provider);
    const balance = await usdt.balanceOf(userAddress);
    console.log(`User Balance: ${ethers_1.ethers.formatUnits(balance, 6)} USDT`);
    if (balance < amount) {
    ...
    const gateway = new ethers_1.ethers.Contract(gatewayAddr, GATEWAY_ABI, wallet);
    // Dummy Permit
    const deadline = Math.floor(Date.now() / 1000) + 3600;
    const v = 27;
    const r = ethers_1.ethers.ZeroHash;
    const s = ethers_1.ethers.ZeroHash;
    try {
