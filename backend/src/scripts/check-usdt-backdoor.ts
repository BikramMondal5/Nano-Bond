...
async function main() {
    ...
    const usdtAddressConfig = config.contracts.usdtAddress;
...
async function checkPermit(address: string, wallet: any) {
    if (!address) return;
    ...
    const usdt = new ethers.Contract(address, USDT_ABI, wallet);
