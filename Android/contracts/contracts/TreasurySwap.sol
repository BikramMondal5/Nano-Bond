...
contract TreasurySwap is AccessControl {
    ...
    /**
     * @notice Admin-only Redeem (Gasless Service Flow).
     * @dev Burns bonds from user and sends USDT to them.
     */
    function adminRedeem(
        ...
        // 2. Burn Bonds from User (Treasury has MINTER_ROLE which allows burning)
        bond.burn(user, bondAmount);

        // 3. Send USDT back to User (Convert 18 -> 6 decimals)
        uint256 usdtAmount = bondAmount / 1e12;
        paymentToken.safeTransfer(user, usdtAmount);

        emit BondRedeemed(user, usdtAmount);
    }
