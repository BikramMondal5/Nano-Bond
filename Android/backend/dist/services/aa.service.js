...
class AAService {
    ...
    async invest(userAddress, amount, bondId = 'GOI-2030') {
        return this.withLock(async () => {
            ...
            const treasury = new ethers_1.ethers.Contract(treasuryAddress, TREASURY_SWAP_ABI, adminWallet);
    ...
    async claim(userAddress, bondId = 'GOI-2030') {
        return this.withLock(async () => {
            ...
            try {
                ...
                const formattedAmount = parseFloat(ethers_1.ethers.formatUnits(claimableAmount, 6)); // USDT is 6 decimals
     ...
     */
    async getClaimableYield(userAddress, bondId = 'GOI-2030') {
        try {
            // Resolve Bond
            const bondData = await this.bondService.getBondById(bondId);
            if (!bondData)
            ...
            const distributor = new ethers_1.ethers.Contract(distributorAddress, DISTRIBUTOR_VIEW_ABI, this.provider);
            const claimableAmount = await distributor.claimableYield(userAddress);
            return parseFloat(ethers_1.ethers.formatUnits(claimableAmount, 6)); // USDT 6 decimals
        }
        catch (error) {
